...
import PerformanceItem from '@components/ItemPerformanceComponent'
...
class PerformancePage extends Component {
  ...
  renderRow({item}) {
    var strDate = item.date.split(" ")[0];
    var d = new Date();
    var currentMonth = d.getMonth()+1;
    var currentDay = d.getDate();
    var currentYear = d.getFullYear();
    var tmp = currentMonth+"/"+currentDay+"/"+currentYear;
    if(tmp === strDate){
      return(
      <PerformanceItem  values={[item.totalTrades, item.floatingTrades, item.GainedPips, item.profitingFactor, strDate, 1]}/>);
    }
    return (
      <PerformanceItem  values={[item.totalTrades, item.floatingTrades, item.GainedPips, item.profitingFactor, strDate, 0]}/>);
  }
  render() {
    return (
